      ...
      <ul class="nav nav-list">
        ...
        <?php
			...
			while($row = mysql_fetch_array($result)) {
				$sql = "SELECT status FROM solve WHERE (username='".$_SESSION['username']."' AND practice_id='".$row['sl']."')";
				$res = mysql_query($sql);
				$tag = "";
				...
				if(isset($_GET['id']) and $_GET['id']==$row['sl']) {
					...
					echo("<li class=\"active\"><a href=\"#\">".$row['name'].$tag."</a></li>\n");
          	        		...
          	        		echo("<li><a href=\"practice.php?id=".$row['sl']."\">".$row['name'].$tag."</a></li>\n");
          	    	}
		}
	?>
      </ul>
      ...
      ?>
      <br/>
      <form action="solve1.php" method="get">
      <input type="hidden" name="id" value="<?php echo($selected['sl']);?>"/>
	<br/>
	<hr/>
      <?php
        // number of people who have solved the problem
        $query = "SELECT * FROM solve WHERE(status=2 AND practice_id='".$selected['sl']."')";
        $result = mysql_query($query);
