<?php
	...
	if (!preg_match("/^[^\\/?* :;{}\\\\]+\\.[^\\/?*: ;{}\\\\]{1,4}$/", $_POST['filename']))
		header("Location: solve1.php?ferror=1&id=".$_POST['id']); // invalid filename
        // check if the user is banned or allowed to submit and SQL Injection checks
        	...
        	if(trim($soln) == "" or trim($filename) == "" or trim($lang) == "")
        		header("Location: solve1.php?derror=1&id=".$_POST['id']);
        	else {
			if($_POST['ctype']=='new')
				// add to database if it is a new submission
				$query = "INSERT INTO `solve` ( `practice_id` , `username`, `soln`, `filename`, `lang`) VALUES ('".$_POST['id']."', '".$_SESSION['username']."', '".$soln."', '".$filename."', '".$lang."')";
			else {
				// update database if it is a re-submission
				$tmp = "SELECT attempts FROM solve WHERE (practice_id='".$_POST['id']."' AND username='".$_SESSION['username']."')";
				$result = mysql_query($tmp);
				$fields = mysql_fetch_array($result);
				$query = "UPDATE solve SET lang='".$lang."', attempts='".($fields['attempts']+1)."', soln='".$soln."', filename='".$filename."' WHERE (username='".$_SESSION['username']."' AND practice_id='".$_POST['id']."')";
			}
			mysql_query($query);
			...
			if($socket) {
				...
				$query = "SELECT time, input, output FROM practice WHERE sl='".$_POST['id']."'";
				$result = mysql_query($query);
				$fields = mysql_fetch_array($result);
				...
				if($status == 0) {
					// compile error
					$query = "UPDATE solve SET status=1 WHERE (username='".$_SESSION['username']."' AND practice_id='".$_POST['id']."')";
					mysql_query($query);
					$_SESSION['cerror'] = trim($contents);
					header("Location: solve1.php?cerror=1&id=".$_POST['id']);
				} else if($status == 1) {
					if(trim($contents) == trim(treat($fields['output']))) {
						// practice solved
						$query = "UPDATE solve SET status=2 WHERE (username='".$_SESSION['username']."' AND practice_id='".$_POST['id']."')";
						mysql_query($query);
					...
					$query = "UPDATE solve SET status=1 WHERE (username='".$_SESSION['username']."' AND practice_id='".$_POST['id']."')";
					mysql_query($query);
					header("Location: solve1.php?terror=1&id=".$_POST['id']);
				}
			} else
				header("Location: solve1.php?serror=1&id=".$_POST['id']); // compiler server not running
		}
	}
?>
